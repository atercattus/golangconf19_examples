...
func (ws *WebsocketClient) SetEventsReceiver(cb EventCallback) {
	ws.commonCb = cb
}
...
func (ws *WebsocketClient) reconnect() (success bool) {
	...
	ws.ws.Call(`addEventListener`, `message`, js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		...
		if err := event.Unmarshal([]byte(data)); err != nil {
			...
			asyncCb(event)
		} else if ws.commonCb != nil {
			ws.commonCb(event)
		} else {
			// иначе просто теряем сообщение
			println(`WS event lost:`, event)
		}
		return nil
	}))
